...
def get_minimal_spanning_subnet(ip_list):
    ...
    lzeros = 32
    while diff_bits:     # diff_bits becomes 0 when the last 1 (from the left) is shifted right
        diff_bits >>= 1  # shift bits to the right
        lzeros -= 1      # decrement lzeros for each bit shift
    print(f"{diff_bits_str}")
    ...
    subnet_mask = (0xFFFFFFFF << (32 - lzeros)) # shift left to keep (lzeros) 1s and (32-lzeros) 0s
    subnet_mask = subnet_mask & 0xFFFFFFFF # keep only the last 32 bits after the shift
    network_address = min_ip & subnet_mask # bitwise AND

    print_ip_line_from_int(min_ip,"(min)")
    print_ip_line_from_int(subnet_mask,"(subnet mask)")
